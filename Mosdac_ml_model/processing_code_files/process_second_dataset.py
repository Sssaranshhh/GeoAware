"""
Process MOSDAC INSAT Second Dataset
====================================

Reads HDF5 files from:
    dataset/raw_second_dataset/

Saves processed output to:
    dataset/second_processed/

Compatible with INSAT L2B OLR / IMC products.
"""
...
class Config:
    ...
    RAW_DATA_DIR = PROJECT_ROOT / "dataset" / "raw_second_dataset"
    PROCESSED_DATA_DIR = PROJECT_ROOT / "dataset" / "second_processed"
    LOG_DIR = PROJECT_ROOT / "logs"

    PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)
    LOG_DIR.mkdir(parents=True, exist_ok=True)

    METADATA_PATH = PROCESSED_DATA_DIR / "metadata_second_dataset.csv"
    LOG_FILE = LOG_DIR / f"second_dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def setup_logger():
    logger = logging.getLogger("SECOND_DATASET_PIPELINE")
...
def extract_2d_slice(data: np.ndarray, logger) -> np.ndarray:
    if data.ndim == 2:
        return data
